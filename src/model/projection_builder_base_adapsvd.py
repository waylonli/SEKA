...
class ProjectionBuilderBase(abc.ABC):
    def __init__(
        ...
        # Multi-GPU support
        multi_gpu = torch.cuda.device_count() > 1 and device == 'cuda'
        
        if multi_gpu:
            print(f"Using {torch.cuda.device_count()} GPUs for projection building")
            self.model = AutoModelForCausalLM.from_pretrained(
                model_path, 
                device_map="auto",
                torch_dtype="auto"
            ).eval()
            self.device = "cuda"  # Override device for multi-GPU
        else:
            self.model = (
    ...
    def _check_attention_increase(self, text_H, text_Hp, indices_H, indices_Hp):
        ...
        inputs_H = self.tokenizer(text_H, return_tensors='pt', add_special_tokens=False)
        inputs_Hp = self.tokenizer(text_Hp, return_tensors='pt', add_special_tokens=False)
        
        # Handle multi-GPU device placement
        device = self.get_model_device()
        inputs_H = inputs_H.to(device)
        inputs_Hp = inputs_Hp.to(device)
    ...
    def run(self, output_dir):
        ...
        for ex in self.iter_examples():
            for ctx, rel_q, ans in self.get_triplets(ex):
                ...
                # Clear GPU cache periodically for multi-GPU setups
        ...
        # Final GPU memory cleanup for multi-GPU setups
    ...
    def _compute_and_save_svd(self, buf_H, buf_Hp, num_layers, n_kv, output_dir):
        ...
        for L in tqdm(range(num_layers), desc="Computing SVD Components", unit="layer"):
            ...
            for h in range(n_kv):
                device = self.get_model_device()
                H_mat = torch.cat(buf_H[L][h], 0).double().to(device)
                Hp_mat = torch.cat(buf_Hp[L][h], 0).double().to(device)
                ...
                try:
                    ...
                    d = Omega_p.shape[0]
                    device = self.get_model_device()
                    Up = torch.eye(d, device=device, dtype=torch.float)
                    Sp = torch.ones(d, device=device, dtype=torch.float)
